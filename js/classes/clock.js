export default class Clock {
  ...
  setRealTime(second, minute, hour, day, month, mDay, year) {
    const time = document.querySelector('[data-id="main-date"]');
    time.innerHTML = '';
    time.innerHTML = `${this.#setDay(day)} ${this.#setDate(
      month,
      mDay,
      year
    )} <br>${this.#setClock(hour, minute, second)}`;
    return `${this.#setDay(day)} ${this.#setDate(
      month,
      mDay,
      year
  ...
  #setClock(hour, minute, second) {
    return `${this.#checkTime(hour)}:${this.#checkTime(
      minute
